...
<template>
  <article :id="project.id" class="project-showcase" :class="{ 'project-showcase--right': Number(project.number) % 2 === 0 }">
    ...
    <figure class="project-showcase__media">
      ...
      <img :src="project.image" :alt="project.imageAlt" width="1440" height="900" loading="lazy" decoding="async" class="project-showcase__image">
      <figcaption class="mono">{{ project.number }} / {{ project.subtitle }}</figcaption>
    </figure>
    <div class="project-showcase__body">
      <div class="project-overview"><h4 class="mono">The problem & approach</h4><p>{{ project.description }}</p><p class="project-impact">{{ project.impact }}</p>
        <h4 class="mono">Technology</h4><div class="project-showcase__tags"><TechTag v-for="technology in project.technologies" :key="technology" :label="technology" /></div>
      </div>
      <div><h4 class="mono">My contribution <span> / {{ project.role }}</span></h4><ul class="project-showcase__highlights"><li v-for="highlight in project.highlights" :key="highlight">{{ highlight }}</li></ul>
